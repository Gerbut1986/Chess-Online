...
namespace ChessRules_ClassLibrary_
{
    class Moves
    {
        ...
        bool CanKnightMove()
        {
            //return fm.AbsDeltaX == 1 && fm.AbsDeltaX == 2 ||    /// here was a mistake
            //       fm.AbsDeltaY == 2 && fm.AbsDeltaY == 1;
            // it's right code:
        ...
        bool CanPawnPromote()  // Проверка, может ли превращаться пешка в QRBN
        {
            if (fm.to.y == 7) // Если это 7я горизонталь
                    // if("QRBN".Contains(fm.promotion)) // если среди них содержиться та самая фигура в которую мы превратились
            ...
            if (fm.to.y == 0) // Если это 7я горизонталь
